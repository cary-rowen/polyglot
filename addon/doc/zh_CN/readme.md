...
Polyglot 使用 Windows 数据保护 API（DPAPI）保护各翻译引擎的 API 密钥、访问令牌和密码。NVDA 配置中保存的是受保护的密文，而不是可直接读取的明文。插件启动或切换 NVDA 配置文件时，会自动加密尚未加密的凭据。

凭据输入框默认隐藏。勾选旁边的“显示凭据”可查看内容。

将 NVDA 便携版复制到另一台电脑，或改用其他 Windows 用户后，已保存的凭据通常无法解密。插件会将其视为未填写，请重新输入凭据并选择“应用”或“确定”。

如果暂时不重新输入，原密文会继续保留。将便携版移回原来的电脑并使用原 Windows 用户后，原凭据通常仍可使用。
